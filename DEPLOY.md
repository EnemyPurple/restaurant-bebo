...
| Фото | Подтягиваются из `assets/bundled/` при каждом деплое |
| База | SQLite на Render (данные сохраняются между перезапусками, сбрасываются при новом деплое) |
| Обновление | `git push` → Render пересобирает автоматически |
